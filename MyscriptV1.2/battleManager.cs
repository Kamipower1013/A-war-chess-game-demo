...
public class battleManager : MonoBehaviour
{
   ...
   //player的状态机开关
    ...
    public int AliveEnemy_count;
    public Text battleRoundCountText;
    public Text EnemyAlive_count;
    ...
    public bool enemy5AIroundFinish;
    //
    ...
    void Start()
    {
        ...
        enemy5AIroundFinish = false;
        Refrash_AliveEnemy_Num();
    }
    ...
    public void Refrash_AliveEnemy_Num()
    {
        Debug.Log("存活敌人数"+ AlltheEnemy.instance.theAliveEnemy_Num());
      EnemyAlive_count.text=AlltheEnemy.instance.theAliveEnemy_Num().ToString();
    }
    ...
    public void Player_battleTime()//在update里
    {
        if (current_state == Battle_State.StartRoundstay)//
        {
            ...
            if (isActive_roundPanel == false)
            {
                ...
                CardManager.instance.isRoundStart = true;
            }
        }
        ...
        {    
            ...
            {   
                ...
                CardManager.instance.Refresh_Status_PenelAbout_All();
        ...
        else if (current_state == Battle_State.player_roundEnd)
        {
           ...
           //行走choice归位
            ...
            int The_FirstAlive_enemy_index = first_aliveEnemy();
            active_true_allEnemyCM();
            CMplayer.SetActive(false);
            Debug.Log(The_FirstAlive_enemy_index + "几号激活");/////////////////////找到第一个存活敌人
            Active_TheFirstAliveenemy(The_FirstAlive_enemy_index);
            current_state = Battle_State.StartRoundstay;
        }
       
    }
    
    public void Active_TheFirstAliveenemy(int enemynum)
    {
        if (enemynum == -1)
        {
        ...
        {
            current_Allbattle_state = Allbattle_state.Enemy5_state;
        }
    }
    ...
    {   
        ...
        {
            enemyList[4].GetComponent<EnemyAI>().startAI = true;
        }
    }
    ...
    public int Count_Alive_EnemyList()
    {
        int count = 0;
        for(int i = 0; i < enemyList.Length; i++)
        {
            bool thisone = judge_enemyAlive(i);
            if (thisone == true)
            {
                count++;
            }
    ...
    void Update()
    {
        ...
        if (Current_Game_State == Game_state.has_enemyAlive)
        {
            ...
            else if (current_Allbattle_state == Allbattle_state.Enemy5_state)
            {
               // Debug.Log("进入Enemy5状态");
                int enemy_index = 5;
                if (whichisalive[enemy_index - 1] == -1)
                {
                    enemy5AIenter = true;
                    enemy5AIroundFinish = true;
                    cameraControlScript.setoff_deadenemy(enemy_index - 1);
                }
                if (enemy5AIenter == false)
                {
                  //  Debug.Log("ENemy4进入AIstart=true");
                    enemy_battleTime(enemy_index);
                ...
                else if (enemy5AIroundFinish == true)
                {
                    int nextAliveEnemy = theNext_aliveEnemy(enemy_index - 1);
                    if (nextAliveEnemy != -1)
                    {
