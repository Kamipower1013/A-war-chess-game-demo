...
public class findTheWayPlayer : MonoBehaviour
{
    ...
    public int TheBuff_addPathStep;
    public bool Ischoicen_Card;
    //public int search_area = 5;
    TextMesh pathnumber;
    bool iniBFSover;
    LinkedList<int> mylist;
     public MapListNode[,] GraphNobeArr;
    ...
    {   
       ...
       GraphNobeArr = new MapListNode[arrayLength, arrayLength];
        ...
        contact_Arr();
        RougeMapCreateScript.Mapscript_About_TreasureBox();
        RougeMapCreateScript.Init_Two_contect_Gate();
        cannotfindplayer = false;
        Ischoicen_Card = false;
    }
    ...
    public void ActivePathroundPlayer()//用来点亮周围的Path
    {
        ...
        while (queue_playerCanMoveto[0].pathcount < numberofStep+TheBuff_addPathStep)//小于步数
        {
    ...
    public  bool thisnodeCanWalk_EnemyWay(MapListNode thisNode)
    {
          ...
          //  Debug.Log("thisnode不等于null");
            ...
            {
                if (thisNode.path_Element != MapType.enemy1_Pos&&thisNode.path_Element!=MapType.enemy2_Pos&&thisNode.path_Element!=MapType.enemy3_Pos&& thisNode.path_Element != MapType.enemy4_Pos && thisNode.path_Element != MapType.enemy5_Pos)
                {
